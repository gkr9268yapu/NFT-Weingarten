...
export default function PlayersPage() {
    ...
    const openStatsEdit = (session: CalendarSession) => {
        ...
        for (const p of players) { const ps = allStats.find(s => s.playerId === p.id); initial[p.id] = ps?.sessions?.[session.id] ?? { attended: false, goals: 0, manOfMatch: 0, mvpPoints: 0 }; }
    ...
    return (
        <div style={{ background: "#070d1a", minHeight: "100dvh" }}>
            ...
            <style>{`
                ...
                /* ── PLAYER LIST ROWS ── */
                .ps-row         { display: grid; grid-template-columns: 80px 1fr 220px; padding: 12px 24px; align-items: center; }
                ...
                @media (max-width: 640px) {
                    ...
                    .ps-row           { grid-template-columns: 44px 1fr auto; padding: 10px 14px; gap: 8px; }
